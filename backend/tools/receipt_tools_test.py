"""Test script for process_receipt_image tool."""
import sys
import os
...
def run_tests():
    ...
    def t2():
        s = SessionLocal()
        r = process_receipt_image(ProcessReceiptImageInput(
            image_data="NR:not-a-receipt-image-data",
            image_filename="photo.jpg"), s)
        assert r.confidence < 0.3
        assert r.status == "unrecognized_image"
        assert r.vendor_name is None
        s.close()

    # Test 3: Blurry/partial receipt
    def t3():
        s = SessionLocal()
        r = process_receipt_image(ProcessReceiptImageInput(
            image_data="BL:blurry-receipt-partial",
            image_filename="receipt.jpg"), s)
    ...
    def t4():
        ...
        try:
            process_receipt_image(ProcessReceiptImageInput(
                image_data="NG:negative-amount-receipt",
                image_filename="receipt.png"), s)
            assert False
    ...
    def t5():
        ...
        try:
            process_receipt_image(ProcessReceiptImageInput(
                image_data="ZD:zero-amount-receipt",
                image_filename="receipt.png"), s)
            assert False
        except ValueError as e:
            assert "Invalid receipt amount" in str(e)
